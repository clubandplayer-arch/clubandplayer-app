...
**6C bis 2026-09-11 — COMPLETATA PER AUDIT/COMPATIBILITY E REVIEW CATALOG ITALIA; RUNTIME NON INIZIATO.** Approvato il modello condizionale `country+sport → organization → organization option`: in Italia/Calcio il menu Ente è necessario e LND è primo; altrove un solo organizer viene auto-selezionato e il menu ridondante può essere omesso, mentre il terzo menu appare soltanto con opzioni applicabili. Il catalogo separa livelli LND, competizioni, coppe, tornei, age category e umbrella, sposta gli enti fuori dal menu categoria piatto, mantiene ELITE soltanto come label giovanile LND, esclude Under 13 e conserva FIP/ENDAS fail-closed. Nessun runtime, form, API, migration, seed, backfill, RLS, Mobile, deploy o Production modificato/autorizzato. Deliverable: [`phase-6c-bis-italy-football-organizer-cascade.md`](european-expansion/phase-6c-bis-italy-football-organizer-cascade.md). 6D non è iniziata né autorizzata.
