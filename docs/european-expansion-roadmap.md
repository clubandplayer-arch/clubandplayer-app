...
**6C bis 2026-09-11 — PREVIEW IN CORSO; PRIMO TRANCHE APPLICATIVO C7 PREPARATO.** Il catalogo Sport web comprende ora 15 opzioni e aggiunge `Calcio a 7` fra `Calcio a 8` e `Futsal`, con mapping canonico `football / association_football / seven_a_side`, ruoli non vuoti, label i18n e categorie C7 separate dal C8. Il catalogo review-only degli ENTI Italia/Calcio ordina `LND → Lega Calcio a 8 → E.I.F.A. → …`, rimuove FIP (riservata alla futura tranche Pallacanestro) e non crea ELITE come organizzatore. Le competizioni C8 2026/27 e C7 2025/26 consegnate dall'utente restano distinte per variant/stagione; quelle C7 storiche non sono dichiarate attivabili per il 2026/27. La matrice dei 15 sport registra copertura e backlog senza dichiarare completata la ricerca internazionale. Migration additiva preparata ma **non applicata**; nessun deploy, merge main, seed o Production. Mobile: impatto documentato, implementazione non eseguita. 6D non è iniziata né autorizzata.
...
**Preview bootstrap repair 2026-09-11 — REPOSITORY COMPLETATO / SUPABASE PENDING.** Riprodotto su PostgreSQL 16 vuoto il `42P01` causato da `20250221090000_add_club_id_to_opportunities.sql`: mancava l'intera baseline pre-history, non soltanto `opportunities`. Aggiunta baseline `20250220000000`, risolti i successivi conflitti d'ordine/versione e resi branch-safe i data step senza indebolire i partial-match guard. L'intera directory migration applica ora da zero e termina con `FULL_MIGRATION_REPLAY_PASS`, inclusa la migration C7, senza connessioni remote. `No repository connected` implica che il branch Unhealthy non usa ancora questa correzione; serve GitHub Integration con working directory `.`, `Deploy to production` disabilitato e ricreazione/reset del solo Preview Branch. Production/history Production non interrogate né modificate; C7 remoto e 6D restano bloccati fino a branch Healthy. Deliverable: [`phase-6-preview-full-history-repair.md`](european-expansion/phase-6-preview-full-history-repair.md).

**Preview Auth replay follow-up 2026-09-11 — FIX COMMITTATO / DEPLOY PREVIEW PENDING.** Il branch GitHub-driven ha superato baseline e `opportunities`, fermandosi su `auth.set_config`, funzione inesistente che il precedente fixture locale aveva erroneamente simulato. Rimossa la funzione fittizia e spostate password minima 12, requisito lower/upper/digits/symbols e OTP email 900s nel meccanismo supportato `supabase/config.toml`; preservato esplicitamente l'Apple `client_id` non segreto senza committare secret. Il replay SQL completo torna PASS. È sufficiente il nuovo deploy automatico dello stesso Preview Branch; reset/SQL Editor non richiesti salvo Retry Preview. Production non modificata; C7 resta sospesa fino a Healthy.

**Preview isolation/C7 smoke 2026-09-11 — BLOCKED DA VERCEL SSO.** Ricevuti Healthy Supabase `fase6-github`, ultima migration C7 e configurazione delle cinque variabili Preview come evidenze user-reported. I probe reali su root, `/api/env`, catalogo e whoami restituiscono tutti `302` verso Vercel SSO: SHA, build post-env, project ref browser/server, credenziali e catalogo non sono verificabili. Nessuno smoke mutativo è stato iniziato. Aggiunto diagnostico `/api/env` privo di secret e piano separato per la credenziale privilegiata esposta. Unico unblock: bypass automation secret trasmesso come secret oppure rimozione temporanea della protezione solo Preview; poi GET gate prima di qualunque save. Production invariata e C7 non va riapplicata. Deliverable: [`phase-6-preview-isolation-and-c7-smoke.md`](european-expansion/phase-6-preview-isolation-and-c7-smoke.md).

**Preview isolation follow-up 2026-09-11 — DEPLOYMENT CANDIDATO / BYPASS PENDING.** Vercel indica Ready `e5f8a4a` con lo stesso titolo del commit locale `bfc8864`, ma la diversa identità Git e l'assenza di remote nel checkout impediscono di dichiararli uguali senza il probe runtime. Aggiunto un gate GET-only fail-closed che richiede lo SHA Vercel, verifica `/api/env`, ref browser/server `jbovlevodfouwuvtdlja`, assenza di fallback, coerenza anon, configurazione e lettura service-role, variante `seven_a_side` e mapping legacy senza stampare segreti o payload. Il bypass va iniettato come Environment variable temporanea dell'ambiente Codex e revocato/rimosso dopo gli smoke. Finché il gate non passa, nessun salvataggio; Production invariata.

**Preview isolation runtime 2026-09-11 — GET-ONLY PASS.** Il branch URL ha risolto il nuovo deployment `036f13d3d39c40efbc2910937e20533efd7d7fe1`. Il gate autenticato ha confermato `mode=preview`, ref browser/server `jbovlevodfouwuvtdlja`, `SUPABASE_URL` senza fallback, anon key coerenti, service-role configurata e funzionante in lettura, variante remota `seven_a_side` e mapping `Calcio a 7`. Nessuna write, migration o operazione Production. Il bypass può essere revocato; gli smoke mutativi restano separati e non eseguiti.

**Preview C7 save/reload smoke 2026-09-11 — PENDING USER-OPERATED BROWSER.** Bypass e variabile agente sono stati revocati correttamente; il probe non autenticato torna `302`. È documentata la procedura browser Vercel-authenticated: riconferma `/api/env` sul deployment Ready prima della write, account Player creato esclusivamente in Supabase `fase6-github`, ordine C8/C7/Futsal, sei ruoli C7, singola PATCH profilo, reload/GET e confronto fra legacy `Calcio a 7` e tre ID canonici. Nessuna migration e nessuna Production. Non registrare PASS finché l'operatore non restituisce gli esiti reali.

**Preview Auth entry 2026-09-11 — GOOGLE BLOCKED / EMAIL PATH READY.** Lo screenshot Google prova `400 redirect_uri_mismatch`: il callback del project ref Preview non è autorizzato nel client OAuth, non un errore delle credenziali utente. Gli utenti Production non sono baseline del branch Auth. Per lo smoke C7 usare un account dedicato creato in Auth Users di `fase6-github` e login email/password; Google resta un follow-up manuale con client Preview dedicato e callback esatto. Corretto inoltre `/signup` per derivare il callback email dall'origin corrente, impedendo il rientro involontario a Production tramite `NEXT_PUBLIC_BASE_URL`. Nessuna configurazione Auth Production modificata.

**Preview C7 first write 2026-09-11 — BLOCKED / DIAGNOSTIC PREPARED.** User-operated smoke: email login, ordine C8/C7/Futsal e sei ruoli PASS; profilo FAIL su `profile_primary_sport_write_failed`; geografia e registry club vuoti. Il codice nasce nel planning canonico prima della write ma maschera correttamente il dettaglio DB: aggiunti `traceId` e log server redatto per correlazione al prossimo tentativo minimo. Preparata fixture manuale Preview-only, fuori dalle migration, con geografia e club sintetici; nessun dato Production/personale. `CSEN` come Categoria C8 conferma la nota limitazione legacy organizzatore/categoria rinviata all'integrazione 6C-bis. Salvataggio/reload ed esperienze non sono PASS.

**Preview C7 mutation diagnostic 2026-09-11 — UPDATE/UPSERT ISOLATED, ROOT DB ERROR PENDING.** Sul deployment `be64af05af6d692be7c8b0145a8b876779a2c323` il form ha mostrato ancora soltanto il codice generico pur includendo la diagnostica del planner: il planner è quindi PASS e il fallimento è nel successivo `profiles.update` o fallback `profiles.upsert`. Estesa la diagnostica Preview redatta a entrambi i rami e aggiunto lo stadio; Production resta generica. Serve un solo retry minimo sul prossimo deployment, senza migration, fixture o esperienze.

**Preview Profile schema repair 2026-09-11 — ROOT CAUSE CONFIRMED / FIX COMMITTED.** Il deployment `7cad19a` ha restituito `update/PGRST204`: `profiles.birth_country` assente dallo schema cache. La baseline aveva ricostruito lo schema versionato ma omesso colonne Profile Edit pre-history già presenti nell'ambiente storico; dopo `birth_country` sarebbero emerse le altre. Aggiunta una migration additiva/null-safe che completa il contratto Profile Edit e ricarica PostgREST, senza righe, backfill, dati Production o riapplicazione C7. Dopo Preview Healthy serve retry minimo C7/Portiere, poi reload, poi esperienze separate. Non è un blocker permanente e Production resta esclusa.

**Preview migration history gate 2026-09-11 — REMOTE COMPARISON PENDING.** Il workflow raggiunge `fase6-github` ma si arresta prima delle due migration pending con `Remote migration versions not found in local migrations directory`. Git prova una rinomina `20260720103000_normalize_pallavolo_to_volley` → `20260720102500...` dovuta alla collisione con `20260720103000_seed_players_from_excel`, ma non prova che sia l'orfana remota. Preparato un report SQL a singolo JSON, transazione read-only/rollback, che confronta le 132 versioni del checkout con history Preview e verifica gli effetti Profile Edit. Vietati reset, placeholder e history marking senza output/effect check. Production non interrogata o modificata; `complete_profile_edit_schema` non applicata.

**Preview migration history audit 2026-09-11 — HISTORY COERENTE / RETRY AUTOMATICO.** Il report eseguito su `fase6-github` restituisce `remoteOnly=[]`, `sameVersionDifferentName=[]` e soltanto `20261211130000_complete_profile_edit_schema` in `localOnly`; la coda remota termina a C7 e gli effetti della nuova migration sono assenti. Non serve né è consentita una repair. Il file pending è tracciato, non ignorato e nel percorso standard; la riconciliazione CLI dello snapshot verificato produce una sola pending. Il vecchio errore implica quindi un checkout/lista locale differente non identificabile dal log privo di SHA. Prossimo passo: nuovo workflow automatico sul commit corrente e controllo dello SHA/file; nessun reset, SQL manuale o Production.

**Preview migration targeted recovery 2026-09-11 — MANUAL APPLY PREPARED.** Anche il retry delle 12:57 fallisce prima dell'apply senza SHA/versioni. Nessun ulteriore commit di solo retry: predisposto runbook atomico esclusivo per `fase6-github` che richiede testa C7, target non registrato e dipendenze presenti, applica le sole colonne della migration canonica, verifica gli effetti e registra `20261211130000` soltanto nella stessa transazione riuscita. Errori causano rollback; nessuna riga profilo/Auth, fixture, account o Production viene modificata.

**Preview profile schema recovery 2026-09-11 — APPLIED / POSTGREST GET PENDING.** L'operatore riporta dal runbook su `jbovlevodfouwuvtdlja`: `20261211130000 complete_profile_edit_schema`, `applied_and_effects_verified`. History ed effetti schema Preview sono coerenti; account e dati test preservati. Prima di qualunque PATCH serve un GET browser autenticato `/api/profiles/me` HTTP 200 con presenza di `data.birth_country` e `data.notify_email_new_message`; poi solo C7/Portiere senza esperienze. Il workflow automatico resta un problema separato e non è dichiarato risolto. Production invariata.

**Preview C7 profile/experience 2026-09-11 — PASS / READ SURFACES BLOCKED.** User-reported PASS per save/reload C7 e singola esperienza fittizia. Search fallisce su grant `athletes_view`; Who to follow HTTP 500 resta da correlare; geografia/registry richiedono sia read contract sia fixture. Preparati audit read-only, migration minima con view security-invoker + RLS/policy SELECT geo, runbook atomico Preview e dataset esclusivamente sintetico. Nessuna service-role come bypass, RLS disabilitata, Auth/Production/reset.

**Preview read-contract audit 2026-09-11 — CONFIRMED / APPLY PENDING.** Audit su `jbovlevodfouwuvtdlja`: view Player security-invoker ma senza SELECT; geo senza SELECT e senza RLS; profili/follows/fan-count autorizzati; tutti i dataset geo/registry vuoti. Il runbook `20261211140000` copre soltanto grant/RLS/policy SELECT e verifica anon+authenticated prima della history. La fixture dati resta il passo successivo separato; Who to follow 500 non è ancora attribuito definitivamente. PASS C7 profilo/esperienza preservati, Production esclusa.

**Preview read contracts 2026-09-11 — APPLIED / FIXTURE PENDING.** Operatore conferma `20261211140000 restore_public_read_contracts` `applied_and_effects_verified` sul ref Preview. Fixture sintetica rafforzata con guard sulla testa, ID dedicati/user_id null e risultato a conteggi; non modifica account, profilo o esperienza test esistenti. Search, geo e Who to follow restano pending fino al fixture PASS e a smoke separati. Workflow automatico e Production invariati.

**Preview synthetic fixture 2026-09-11 — APPLIED / THREE READ SMOKES PENDING.** Operatore conferma sul ref Preview `fixture_applied_and_verified`: geo 1/1/1, profili sintetici 2, registry club 2. Prossimo gate browser unico: Search del Player sintetico, cascata Preview Test senza save, Chi seguire senza HTTP 500. PASS profilo/esperienza C7 preservati; nessuna Production.

**Preview read smokes 2026-09-11 — SEARCH/GEO PASS, SUGGESTIONS 500.** Search trova il Player sintetico; cascata Preview Test e salvataggio profilo funzionano. Who to follow fallisce ancora su feed/discover. Aggiunta diagnostica GET Preview-only redatta e a stadi granulari per distinguere candidate profiles, fan counts, athlete view e club verification senza ID o messaggi DB. PASS C7 profilo/esperienza preservati; Production invariata.

**Preview suggestions diagnostic 2026-09-11 — VIEWER GEOGRAPHY RLS DENIED.** Runtime restituisce `500/RLS_DENIED/viewerGeography/42501`, prima di candidati e view Player. Preparato audit read-only mirato a preferences, country/area interests e cataloghi countries/geo_areas, con grant/RLS/policy counts e zero righe personali. Nessun permesso viene ampliato prima di identificare la relazione esatta; tutti i PASS precedenti restano acquisiti.

**Preview viewer geography audit 2026-09-11 — EXACT MISSING GRANT FOUND.** Solo `profile_geo_area_interests` manca di authenticated SELECT; RLS e policy owner sono presenti, e le altre dipendenze sono autorizzate. Preparata migration/runbook `20261211150000` che aggiunge esclusivamente quel grant, fail-closed se RLS/policy mancano e registra history dopo effect check. Nessun anon, DML, service-role o Production; suggestion retry pending.

**Preview viewer geography grant 2026-09-11 — APPLIED / SUGGESTION RETRY PENDING.** Operatore conferma `20261211150000 restore_profile_geo_area_interest_read` `applied_and_effects_verified` sul ref Preview. Prossimo gate: un solo GET autenticato Who to follow, senza nuove write o SQL. PASS richiede 200/ok, lista non vuota e Player sintetico; tutti gli altri PASS restano acquisiti e Production è invariata.

**Preview Who to follow 2026-09-11 — HTTP/RLS PASS / FIXTURE PLAYER RETRY.** GET restituisce 200/ok/count 1/complete senza errori: il 500 è risolto. Il Player fixture è escluso perché `Preview C7 Test Player` contiene una cifra vietata nei nomi persona; il Club è il suggerimento valido. Corretto idempotentemente il solo nome in `Preview Test Player`, stesso UUID e user_id null. Serve re-apply fixture e ultimo GET; nessuna migration, account test o Production.

**Preview C7 smoke package 2026-09-11 — COMPLETE / PHASE 6 STILL PREVIEW.** Fixture corretta PASS; Chi seguire mostra il profilo sintetico, Follow riesce e Profili che segui lo rilegge correttamente. Completati profilo/ruolo C7, esperienza, Search, cascata geo, registry, suggestions e follow persistence. Nessun altro smoke o SQL richiesto. Prossimo lavoro separato: ripristinare il workflow automatico migrations prima di ulteriori migration; 6D, main e Production restano non autorizzati.
