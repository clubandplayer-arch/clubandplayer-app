...
**B4.3 — Transactional server write: IMPLEMENTAZIONE E REVISIONE STATICA COMPLETATE; CERTIFICAZIONE RUNTIME BLOCKED — ISOLATED DATABASE NOT AVAILABLE.** È stata creata la migration additiva della RPC e il wrapper server, senza applicare la migration e senza eseguire write. Il prossimo passo sicuro è predisporre un database isolato e completare il runtime gate B4.3. B4.4 resta **NOT STARTED** e non deve iniziare prima del superamento del gate o di una nuova decisione esplicita; prima di qualsiasi applicazione migration o test manuale mutativo resta inoltre obbligatorio fermarsi e ottenere una nuova approvazione.
