...
**B4.3 — Transactional server write: IMPLEMENTAZIONE E REVISIONE STATICA COMPLETATE; LOCAL POSTGRESQL RUNTIME PASSED; SUPABASE CERTIFICATION BLOCKED.** La migration e il wrapper sono stati validati con fixture sintetiche in PostgreSQL 16 locale senza connessioni remote; la migration non è stata applicata a Preview, Staging o Production. Il prossimo passo sicuro è predisporre un Supabase Branch/Staging isolato e completare il gate sullo schema reale. B4.4 resta **NOT STARTED** e non deve iniziare prima del superamento del gate o di una nuova decisione esplicita; prima di qualsiasi applicazione migration o test manuale mutativo remoto resta inoltre obbligatorio fermarsi e ottenere una nuova approvazione.
