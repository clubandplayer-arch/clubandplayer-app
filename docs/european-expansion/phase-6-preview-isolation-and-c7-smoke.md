...
## Riconciliazione finale del workflow migrations

La testa Preview verificata manualmente è ora `20261211150000`: gli effetti e le righe history delle migration `130000`, `140000` e `150000` sono stati verificati atomicamente dai rispettivi runbook. Non esiste più una migration locale pending da usare come retry del workflow, e non va aggiunta una migration vuota o diagnostica.

Il report read-only `scripts/sports/reports/phase-6-preview-migration-history-audit-read-only.sql` è stato riallineato all'intera directory corrente. Oltre ai confronti per insieme e nome, restituisce ora `localVersionOrder`, `remoteVersionOrder` e `orderedVersionsMatch`, usando ordinamento testuale `C`: ciò rende verificabile lo stesso aspetto rilevante per la riconciliazione CLI anche in presenza dei timestamp storici a 8, 12 e 14 cifre. I test fissano esplicitamente che le versioni sono stringhe opache e che, per i prefissi esistenti, `20251126` precede `20251126120000` e `202605240001` precede `20260524000150`.

Il prossimo controllo è eseguire una sola volta questo report nel SQL Editor della Preview `jbovlevodfouwuvtdlja`. Il prerequisito per ritentare il workflow automatico è: `remoteOnly=[]`, `localOnly=[]`, `sameVersionDifferentName=[]` e `orderedVersionsMatch=true`, con entrambe le sequenze terminate da `20261211150000`. Se il controllo passa ma il workflow continua a produrre `Remote migration versions not found in local migrations directory`, il database e lo storico non sono la causa: il problema è nel checkout/configurazione dell'integrazione GitHub e va corretto lì, senza repair, reset o nuove migration. Se non passa, si interrompe ogni retry e si usa la prima differenza delle due sequenze per una diagnosi mirata. Nessuna di queste operazioni coinvolge Production.

Audit finale user-reported PASS sulla Preview `jbovlevodfouwuvtdlja`: `remoteOnly=[]`, `localOnly=[]`, `sameVersionDifferentName=[]`, `orderedVersionsMatch=true`; entrambe le sequenze terminano a `20261211150000` e tutte le colonne Profile Edit risultano presenti. Database, effetti e migration history sono quindi coerenti. Il prossimo passo è un solo nuovo run del workflow migrations del branch Git collegato, senza commit di retry e senza modifiche SQL. Il risultato atteso è un run Healthy/no-op, perché non esistono migration pending. Se ricompare lo stesso errore, è confermata un'anomalia del checkout/configurazione dell'integrazione e non va eseguita alcuna migration repair.

Il nuovo run ha riprodotto l'errore e la dashboard mostra come ultima migration remota `restore_profile_geo_area_interest_read`. L'audit prova che la Preview contiene `20261211140000` e `20261211150000`; l'errore CLI prova invece che almeno una versione remota non esiste nel checkout clonato dal ref Git `codex/implementare-categorie-sportive-per-paesi`. Il controllo locale dell'agente non può certificare quel ref privato perché il workspace è sul branch `work` e non dispone di remote Git o credenziali GitHub. L'ipotesi verificabile più precisa è quindi che il branch GitHub collegato non contenga entrambi i file `supabase/migrations/20261211140000_restore_public_read_contracts.sql` e `supabase/migrations/20261211150000_restore_profile_geo_area_interest_read.sql`, benché le relative versioni siano state registrate manualmente nella Preview. Il prossimo passo non è un altro retry: va verificata nel tree GitHub dello stesso branch la presenza di questi due file e, se assenti, vanno portati su quel branch insieme ai commit che li introducono. Solo dopo questa correzione del checkout si esegue un singolo workflow no-op. Nessuna modifica alla history database è necessaria o autorizzata.

L'operatore ha verificato che entrambi i file sono presenti anche nel tree GitHub del branch esatto. Sono quindi esclusi sia una discordanza della migration history sia l'assenza dei due file attesi nel repository collegato. Poiché il workflow gestito non espone né SHA del checkout né elenco delle versioni che la CLI considera mancanti, l'ulteriore diagnosi richiede i log interni dell'integrazione Supabase: non è ottenibile con un'altra migration, un retry o un intervento sul database. L'escalation deve includere soltanto project ref `jbovlevodfouwuvtdlja`, branch `codex/implementare-categorie-sportive-per-paesi`, timestamp del run `2026-09-11 15:07:41 UTC`, errore esatto, audit con gli array vuoti e `orderedVersionsMatch=true`, e conferma che le versioni `20261211140000`/`20261211150000` esistono in DB e nel tree Git. Va richiesto a Supabase di comunicare SHA effettivamente clonato, working directory migrations e versioni remote dichiarate mancanti. Fino alla risposta non si scollega o ricrea il branch, non si ripara la history e non si modifica Production.
