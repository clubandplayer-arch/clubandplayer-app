...
begin
 ...
 if to_regclass('public.club_honors') is null then raise exception 'missing club_honors'; end if;
 if not exists(select 1 from pg_policies where schemaname='public' and tablename='club_honors') then raise exception 'missing club_honors RLS policies'; end if;
 if not exists(select 1 from pg_trigger where tgname='validate_club_honor' and not tgisinternal) then raise exception 'missing club_honors validation trigger'; end if;
end $$;
