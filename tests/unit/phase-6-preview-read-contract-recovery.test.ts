...
test('manual recovery is guarded, verifies effects, and records the exact version last', () => {
  ...
  assert.match(runbook, /has_table_privilege\('anon', 'public\.athletes_view', 'select'\)/)
  assert.match(runbook, /c\.relname in \('regions', 'provinces', 'municipalities'\)/)
  assert.match(runbook, /and not c\.relrowsecurity/)
