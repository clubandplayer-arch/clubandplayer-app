          ...
          <tbody className="divide-y">
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="p-3 align-top"><div className="flex min-w-56 flex-wrap gap-2"><button disabled={Boolean(saving)} onClick={() => void act(row.id, 'approve_name')} className="rounded-full bg-emerald-600 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">Approva nome</button><button disabled={Boolean(saving)} onClick={() => void act(row.id, 'reject_name')} className="rounded-full bg-amber-600 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">Rifiuta nome</button>{row.profile_visibility_status === 'suspended' ? <button disabled={Boolean(saving)} onClick={() => void act(row.id, 'restore')} className="rounded-full bg-cyan-700 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">Ripristina</button> : <button disabled={Boolean(saving)} onClick={() => void act(row.id, 'suspend')} className="rounded-full bg-red-700 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">Sospendi</button>}</div></td>
              </tr>
            ))}
            {!rows.length && <tr><td colSpan={4} className="p-6 text-center text-neutral-500">Nessuna anomalia rilevata.</td></tr>}
          </tbody>
        </table>
      </div>
    </main>
