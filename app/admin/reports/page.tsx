...
export default function AdminReportsPage() {
  ...
  const load = async () => {
    ...
    if (q.trim()) {
      // Supabase non supporta OR con ilike in un'unica chiamata semplice, facciamo due tentativi e uniamo lato client
      ...
      // il count di entrambe è quello della rispettiva query; per semplicità mostriamo il maggiore
  ...
  useEffect(() => {
    // reset pagina quando cambiano filtri o query
    setPage(1);
  }, [status, rtype, q]);

  useEffect(() => {
    if (!loadingMe && me?.is_admin) {
      // carica dati quando siamo sicuri che l'utente è admin (o almeno loggato)
      load();
    }
  ...
  const toggleStatus = async (id: string, current: 'open' | 'closed') => {
    if (
      !confirm(
        `Sei sicuro di voler ${current === 'open' ? 'chiudere' : 'riaprire'} la segnalazione?`,
      )
    )
      return;
    setSavingId(id);
    ...
    // ricarica senza perdere i filtri
    load();
  };

  // Guard UI
  if (loadingMe) {
    return <div className="mx-auto max-w-6xl p-4">Caricamento…</div>;
  }
  if (!me) {
    return (
      <div className="mx-auto max-w-6xl p-4 text-red-700">
  ...
  return (
    <div className="mx-auto max-w-6xl p-4">
      <h1 className="mb-4 text-2xl font-semibold">Moderazione — Segnalazioni</h1>

      {/* FILTRI */}
      <div className="mb-4 flex flex-wrap items-end gap-3">
        <div className="flex flex-col">
          <label className="mb-1 text-sm text-gray-600">Stato</label>
          <select
            value={status}
        ...
        <div className="flex flex-col">
          <label className="mb-1 text-sm text-gray-600">Tipo</label>
          <select
            value={rtype}
        ...
        <div className="flex flex-1 flex-col">
          <label className="mb-1 text-sm text-gray-600">Cerca (motivo / note)</label>
          <input
            value={q}
      ...
      <div className="overflow-hidden rounded-lg border">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-600">
            <tr>
              <th className="w-36 px-3 py-2 text-left">Creato</th>
              <th className="w-28 px-3 py-2 text-left">Stato</th>
              <th className="w-32 px-3 py-2 text-left">Tipo</th>
              <th className="px-3 py-2 text-left">Motivo</th>
              <th className="px-3 py-2 text-left">Note</th>
              <th className="w-40 px-3 py-2 text-left">Target</th>
              <th className="w-28 px-3 py-2 text-left">Azione</th>
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={r.id} className="border-t">
                  ...
                  <td className="px-3 py-2">
                    <span
                      className={
                        r.status === 'open'
                          ? 'inline-block rounded-full bg-yellow-100 px-2 py-0.5 text-yellow-800'
                          : 'inline-block rounded-full bg-green-100 px-2 py-0.5 text-green-800'
                      }
                    >
                      {r.status}
                    </span>
