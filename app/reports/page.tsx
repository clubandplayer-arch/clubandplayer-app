...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: reps, error: e1 } = await supabase
        .from('reports')
        .select('id, created_at, status, reason, description, target_type, target_id')
        .eq('user_id', user.id)
  ...
  return (
    <main style={{ maxWidth: 960, margin: '24px auto', padding: '0 16px' }}>
      ...
      {errorMsg && <p style={{ color: '#b91c1c', marginTop: 8, marginBottom: 16 }}>{errorMsg}</p>}
      ...
      {reports.length === 0 ? (
        ...
        >
          ...
          <p style={{ margin: '4px 0 0' }}>
            Puoi segnalarle dal pulsante <b>“Segnala”</b> presente sulle opportunità.
          </p>
        </div>
      ) : (
        <div style={{ display: 'grid', gap: 12, marginTop: 12 }}>
          {reports.map((r) => {
            ...
            const opp = r.target_type === 'opportunity' ? oppsMap[r.target_id] : undefined;
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {badge(r.status)}
                    <span style={{ fontSize: 13, opacity: 0.7 }}>{created.toLocaleString()}</span>
                  </div>
