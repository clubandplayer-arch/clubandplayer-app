...
export default function ClubPostsPage() {
  ...
  const toggleStatus = async (id: string, current: 'aperto' | 'chiuso') => {
    ...
    if (error) {
      setMsg(`Errore aggiornamento: ${error.message}`);
      return;
    }
            ...
            >
              <div>
                <div style={{ fontWeight: 600 }}>
                  {o.title} — {o.city} — {o.role} ·{' '}
                  <span
                    style={{
                      textTransform: 'uppercase',
                      fontSize: 12,
                      color: o.status === 'aperto' ? '#059669' : '#b91c1c',
                    }}
                  >
                    {o.status}
                  </span>
                ...
                <div style={{ fontSize: 12, opacity: 0.7 }}>
                  Creato: {new Date(o.created_at).toLocaleString()} · Ultimo aggiornamento:{' '}
                  {new Date(o.updated_at).toLocaleString()}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <Link
                  href={`/club/posts/edit/${o.id}`}
