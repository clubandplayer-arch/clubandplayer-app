...
export const DELETE = withAuth(async (_req, { supabase, user }, routeContext) => {
  ...
  const id = params?.id ?? '';
  if (!UUID_REGEX.test(id)) return jsonError('ID target non valido', 400);

  const adminClient = getSupabaseAdminClientOrNull();
  if (!adminClient) return jsonError('Service role non configurato', 500);

  const { error } = await adminClient.from('ad_targets').delete().eq('id', id);
  if (error) {
    console.error('[admin/ads/targets] delete error', error);
    return jsonError('Errore durante eliminazione target', 400);
  }
