...
export const GET = withAuth(async (_req: NextRequest, { supabase, user }, routeContext) => {
  ...
  const { data, error } = await adminClient.from('institution_verification_requests').select('document_path').eq('id', id).maybeSingle();
  if (error) return jsonError(error.message, 400);
  const path = data?.document_path as string | null;
  if (!path) return jsonError('Documento non disponibile', 404);
  const { data: signed, error: signedError } = await adminClient.storage.from(BUCKET).createSignedUrl(path, 60 * 5);
