...
export const GET = withAuth(async (_req: NextRequest, { supabase, user }, routeContext) => {
  ...
  try {
    ...
    const { data: message, error } = await supabase.from('direct_messages')
      .select('sender_profile_id, recipient_profile_id, voice_path, deleted_at').eq('id', messageId).maybeSingle();
    if (error) throw error;
    if (!message || message.deleted_at || !message.voice_path) return notFoundResponse('Messaggio vocale non trovato');
    if (message.sender_profile_id !== me.id && message.recipient_profile_id !== me.id) return notAuthorized();
    const { data, error: signedError } = await supabase.storage.from('direct-message-audio')
      .createSignedUrl(message.voice_path as string, 60 * 10);
    if (signedError || !data?.signedUrl) throw signedError || new Error('Audio non disponibile');
    return NextResponse.redirect(data.signedUrl);
  } catch (error) {
    return unknownError({ endpoint: 'direct-messages/voice', error });
  }
});
