...
export const POST = withAuth(async (req: NextRequest, { supabase, user }) => {
  ...
  const payload = {
    ...
    email: clean(body.email), pec: clean(body.pec), website: normalizeWebsite(body.website) || null, representative: clean(body.representative), document_type: clean(body.documentType),
  };
  ...
  if (!payload.entity_name || !payload.fiscal_code || !payload.vat_number || !payload.email || !payload.pec || !payload.website || !payload.representative) return jsonError('Compila tutti i campi obbligatori', 400);
