...
export const GET = withAuth(async (_req, { supabase, user }) => {
  ...
  const { data, error } = await supabase.from('club_honors').select(select).eq('club_profile_id', club.id).eq('is_active', true).order('season', { ascending: false }).order('placement').order('created_at');
...
export const POST = withAuth(async (req: NextRequest, { supabase, user }) => {
  ...
  if (!validSeason(body.season) || !validPlacement(body.placement)) return errorJson('invalid_honor', 400);
  try {
