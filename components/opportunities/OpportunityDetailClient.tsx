...
export default function OpportunityDetailClient({ id }: { id: string }) {
  ...
  const isOwner = useMemo(() => !!meId && !!opp && opp.created_by === meId, [meId, opp]);
  const showApply = role === 'athlete' && !isOwner;

  if (loading) return <div className="p-4">Caricamento…</div>;
  if (err) return <div className="rounded-xl border bg-red-50 p-4 text-red-700">{err}</div>;
      ...
      <div className="rounded-xl border bg-white p-4">
        <header className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {showApply && <ApplyCell opportunityId={opp.id} ownerId={opp.created_by ?? null} />}
          </div>
        </header>
        ...
        <footer className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-gray-600">
          <div className="flex items-center gap-2">
            ...
            {opp.created_by && (
              <FollowButton
                clubId={opp.created_by}
                clubName={opp.club_name ?? undefined}
                size="md"
              />
            )}
          </div>
