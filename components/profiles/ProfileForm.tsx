...
export default function ProfileForm({
  ...
  const [countryFree, setCountryFree] = useState<string>(
    initial?.country && !COUNTRIES.find((c) => c.label === initial.country)
      ? (initial?.country ?? '')
      : '',
  );
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div>
          <label className="mb-1 block text-sm font-medium">Tipo profilo</label>
          <select
        ...
        <div className="md:col-span-2">
          <label className="mb-1 block text-sm font-medium">Nome profilo *</label>
          <input
            className="w-full rounded-xl border px-3 py-2"
            value={displayName}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          ...
          <input
            ...
            placeholder="Es: Attaccante Serie D"
          />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium">Avatar (URL firmato)</label>
          <input
            className="w-full rounded-xl border px-3 py-2"
            value={avatarUrl ?? ''}
