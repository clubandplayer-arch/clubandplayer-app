  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        let cityName = (
          isClubProfile
            ? (j?.city || j?.interest_city || '')
            : (j?.interest_city || '')
        ).trim();
        let regionName = (
          isClubProfile
            ? (j?.region || j?.province || j?.interest_region || j?.interest_province || '')
            : (j?.interest_region || j?.interest_province || '')
        ).trim();

        if ((isClubProfile && (!cityName || !regionName) && (j?.interest_municipality_id || j?.interest_province_id || j?.interest_region_id))
          || (!isClubProfile && (j?.interest_municipality_id || j?.interest_province_id || j?.interest_region_id))) {
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="relative w-3/4 max-w-[140px] aspect-square overflow-hidden rounded-full bg-transparent ring-1 ring-white/70 md:w-2/3 md:max-w-[160px]">
          {p?.avatar_url ? (
            ...
            <div className="h-full w-full rounded-full bg-gradient-to-br from-gray-100 to-gray-200" />
          )}
        </div>
