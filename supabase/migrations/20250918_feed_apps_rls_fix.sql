-- Ensure feed posts/media columns and RLS plus applications club_id and storage policies
begin;

-- POSTS: add media columns if missing and keep RLS permissive for authenticated authors
...
-- APPLICATIONS: add club_id and align RLS so club owners and athletes can view their own
alter table if exists public.applications add column if not exists club_id uuid;

-- backfill club_id from opportunities.owner_id/created_by when missing
update public.applications a
set club_id = coalesce(o.owner_id, o.created_by)
from public.opportunities o
where a.opportunity_id = o.id and a.club_id is null;
...
-- STORAGE buckets: ensure buckets exist and policies allow user-owned folders
