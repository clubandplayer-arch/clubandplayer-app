...
-- B4.4 database-level canary. Empty by default and deliberately contains no
-- Production user IDs; membership is managed as a separate operational step.
create table public.profile_residence_write_canary_users (
  user_id uuid primary key references auth.users(id) on delete cascade,
  created_at timestamptz not null default now()
);

alter table public.profile_residence_write_canary_users enable row level security;

revoke all on table public.profile_residence_write_canary_users from public;
revoke all on table public.profile_residence_write_canary_users from anon;
revoke all on table public.profile_residence_write_canary_users from authenticated;
grant select on table public.profile_residence_write_canary_users to authenticated;

drop policy if exists profile_residence_write_canary_select_own
  on public.profile_residence_write_canary_users;
create policy profile_residence_write_canary_select_own
  on public.profile_residence_write_canary_users
  for select
  to authenticated
...
begin
  ...
  if not exists (
    select 1
    from public.profile_residence_write_canary_users canary
    where canary.user_id = v_uid
  ) then
    raise exception using errcode = '42501', message = 'profile residence write is not enabled for authenticated user';
  end if;
...
comment on function public.update_my_profile_residence(uuid, uuid) is
  'Canary-restricted owner-only atomic canonical and legacy residence dual-write for athlete/staff profiles.';
