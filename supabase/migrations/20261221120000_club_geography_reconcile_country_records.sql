...
-- Allow a Club to move to another supported country without deleting its history.
-- Registrations and honors tied to the previous country are archived atomically.
...
begin
  ...
  -- Country-scoped sporting records cannot remain active after a move. Archive
  -- incompatible rows in the same transaction, preserving their history while
  -- allowing the owner to save the new headquarters and add new registrations.
  update public.club_sport_registrations r
  set is_active = false,
      is_primary = false,
      updated_at = now()
  from public.sports_organization_categories c
  where c.id = r.sports_organization_category_id
    and r.club_profile_id = v_profile_id
    and r.is_active = true
    and c.country_id is distinct from p_residence_country_id;

  update public.club_honors h
  set is_active = false,
      updated_at = now()
  from public.sports_organization_categories c
  where c.id = h.sports_organization_category_id
    and h.club_profile_id = v_profile_id
...
comment on function public.update_my_club_geography(uuid, uuid) is
  'Owner-only atomic Club geography write that archives sporting records incompatible with a new country.';
